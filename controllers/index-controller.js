...
class Index {
    ...
    /**
     * @RequestMapping(path="/upload", method="get")
     */
    uploadIndex(req, res) {
    ...
    /**
     * @RequestMapping(path="/info.do", method="get")
     */
    infoIndex(req, res) {
        logger.debug('infoIndex called for path /info.do');
        ...
        util.render(req, res, 'info_index', context);
    }


    /**
     * @RequestMapping(path="/survey-index.do", method="get,post")
     */
    surveyIndex(req, res) {
        logger.debug('surveyIndex called for path /survey-index.do');

        const params = param.parse(req);
        const context = params;
        logger.debug('Context for rendering -> ' + JSON.stringify(context));

        if (typeof(context.authenticated) != 'undefined') {
            let authString = "";
            authString += `<script>`;
            authString += `$(() => {`;
            authString += `  authenticated = true;`;
            authString += `  sessionUser = {`;
            authString += `    id:'${context.userId}',`;
            authString += `    name:'${context.userName}'`;
            authString += `  };`;

            if (typeof(context.page) != 'undefined') {
                authString += `  showPage('${context.page}');`;
            }

            authString += `})`;
            authString += `</script>`;

            context.authString = authString;
        }

        util.render(req, res, 'survey_index', context);
    }


    /**
     * @RequestMapping(path="/survey-creator.do", method="get,post")
     */
    surveyCreatorIndex(req, res) {
        logger.debug('surveyCreatorIndex called for path /survey-creator.do');
        ...
        util.render(req, res, 'survey-creator-web', context);
    }
